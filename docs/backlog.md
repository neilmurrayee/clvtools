...
## 34. `[x]` Re-verdict Appendix 3, then work what is genuinely weak — round 5
...
### Batch 12 — the last six, and the round closes

`M-09` is the fourth case this round of a **deliberate choice recorded in only
one of the two places the house rule names**. The GGom/NBD's `CET` changed after
Adler's erratum (issue #206) and no longer matches the original MATLAB; the
suite pinned the post-erratum value to 1e-6 and said nowhere *which* value that
was or why, so a reader comparing against the paper would find a disagreement
with no way to tell a deviation from a bug. Now in the README's findings.

`F-11` distinguishes two claims that had been treated as one: other SciPy
methods are **accepted** — item 31's `options_for` validates nothing for a
method it does not recognise, rather than refusing one SciPy might take — and
whether they **work** was never tried. Powell does, converging to within 2
log-likelihood units of the other two on a flat ridge. `F-09`'s finiteness sweep
holds across estimates, standard errors and every numeric column of `predict()`.

`PR-04` holds **bit-identically**: a fit applied to other data gives the sampled
customers exactly the rows they get alone, which is the right bound because
nothing in the fit depends on who is being predicted for. `B-12` holds too —
every resampled draw keeps the cohort's estimation end, so the interval is built
from answers to one question rather than several.

`B-14` is a divergence: `predict` takes no `uncertainty=` argument, because
intervals are composed by the caller from `bootstrap_apply` and
`confidence_intervals` — the same choice the README already records for
`clv.bootstrapped.apply`. Pinned so it reads as a decision.

**Round 5 is closed.** Appendix 3's weak count is **76 → 1**, and the one that
remains is `I-05`, a capability gap carried as item 35. The section-verdict
summary in `docs/spec-audit.md`, left openly stale since batch 1 rather than
half-recounted, is now recomputed: 118 covered, 69 absent, 12 out-of-scope, 1
weak.
