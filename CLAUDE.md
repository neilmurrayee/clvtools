...
Two committed fixtures are **not** reachable from that list.
`tests/fixtures/time_elapsed.csv` and `time_add_periods.csv` — the 840 spans and
280 additions `tests/test_timeunit.py` reads — were generated from CLVTools'
`clv.time` classes in commit `e385a16` by a script that was never committed, so
they cannot be re-baselined today. `docs/backlog.md` item 25 carries writing
that generator; until then, treat those two as the one place "generated by
`tools/oracle/*.R`" is not literally true.
